...
const UserRegistrationNotification = () => {
  ...
  const fetchNotifications = async (isPolling = false) => {
    try {
      const response = await api.get('/goodmood/notifications/type/USER_REGISTRATION');
      if (response.data.status) {
        const newData = response.data.data;
        
        if (isPolling && newData.length > prevCountRef.current) {
          const newItemsCount = newData.length - prevCountRef.current;
          toast.info(`You have ${newItemsCount} new user registration(s)!`);
        }
  ...
  const markAsRead = async (id) => {
    try {
      ...
      if (response.status === 200 || (response.data && response.data.status)) {
        ...
        dispatch(decrementCount({ type: 'USER_REGISTRATION' }));
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="ml-4 bg-purple-100 text-purple-800 text-xs font-bold px-2.5 py-0.5 rounded-full">
        ...
        {loading ? (
          ...
          <div className="bg-white p-12 text-center rounded-xl shadow-sm border border-gray-100">
            <p className="text-gray-500 text-lg">All caught up! No new user registrations.</p>
          </div>
        ) : (
              ...
              <div key={notification.id} className={`p-5 rounded-xl border transition-all ${
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1">
                    <h3 className="font-bold text-purple-900 text-lg">{notification.title || "New User Registration"}</h3>
