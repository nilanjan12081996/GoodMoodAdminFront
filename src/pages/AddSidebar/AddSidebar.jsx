...
import { fetchMasterSidebars } from '../../Reducer/AddSidebarSlice';

export default function AddSidebarForm() {
  const dispatch = useDispatch();
  const { masterSidebars, loading } = useSelector((state) => state.sidebar);
  ...
  // Sync Name when ID is selected
  const handleMasterIdChange = (e) => {
    const value = e.target.value;
    const selectedItem = masterSidebars?.find(
      (item) => String(item.id || item.masterSidebarId || item.master_sidebar_id || item._id) === String(value)
    );

    const foundName = selectedItem 
      ? (selectedItem.masterSidebarName || selectedItem.name || selectedItem.master_sidebar_name || selectedItem.title || selectedItem.sidebarName || '') 
      : '';

    setValue('masterSidebarName', foundName, { shouldValidate: true });
  };
              ...
              <div>
                ...
                >
                  <option value="" disabled>
                    {loading ? 'Loading Names...' : 'Select Master Sidebar Name'}
                  </option>
